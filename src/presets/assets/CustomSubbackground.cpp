#include "CustomSubbackground.hpp"

CustomSubbackground::CustomSubbackground(const char* id)
    : subbg_id(id), subbg_sprite(""), subbg_rect(CCRectZero), subbg_name("") {}

const char* CustomSubbackground::getID() const { return subbg_id; }
const char* CustomSubbackground::getSprite() const { return subbg_sprite; }
const CCRect& CustomSubbackground::getRect() const { return subbg_rect; }
const char* CustomSubbackground::getName() const { return subbg_name; }

CustomSubbackground* CustomSubbackground::registerSubbackground(
    const char* id,
    const char* sprite,
    const CCRect& rect,
    const char* name
    ...
    auto* bg = new CustomSubbackground(id);
...
CustomSubbackground* CustomSubbackground::get(const char* id) {
    auto it = subbackgroundRegistry.find(id);
    return it != subbackgroundRegistry.end() ? it->second : nullptr;
}

const std::map<std::string, CustomSubbackground*>& CustomSubbackground::getAllSubbackgrounds() {
    return subbackgroundRegistry;
}

const std::vector<std::string>& CustomSubbackground::getOrderedKeys() {
    return subbackgroundOrderList;
}

void CustomSubbackground::init() {
    // Special
